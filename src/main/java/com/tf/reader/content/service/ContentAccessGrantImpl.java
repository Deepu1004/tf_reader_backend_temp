...
class ContentAccessGrantImpl implements ContentAccessGrant {
    ...
    // Real posting counts from the generated index fixtures (see EPUB_SMALL_INDEX_FIXTURE's
    // comment), not a guess — the two small books have very different sizes, so one shared
    // constant was never going to be right for both.
    private static final int EPUB_TERM_COUNT = 2_139;
    private static final int PDF_TERM_COUNT = 153;
    ...
    // Real search-index fixtures, one per small book — generated by decrypting the content
    // fixture above under MOCK_BEK, running it through the app's own extractor/buildIndex
    // pipeline (so postings/CFIs genuinely resolve against this fixture's content), and
    // re-encrypting the resulting BookSearchIndex JSON under the same key. Before these existed,
    // `grant()` pointed IndexUrl at `content.url()` itself — the app then decrypted the BOOK's
    // own ciphertext and tried to parse it as index JSON, which fails `utf8Decode` every time
    // (confirmed on-device and via direct curl, 2026-09-01). Only the two small fixtures have one
    // — the big measurement fixtures have no index and `resolveIndexFixture` returns null for
    // them, which is a correct "no index" rather than the wrong book's bytes.
    private static final String EPUB_SMALL_INDEX_FIXTURE = "static/mock-content/sample-small.epub.index.enc";
    private static final String PDF_SMALL_INDEX_FIXTURE  = "static/mock-content/sample-small.pdf.index.enc";
    ...
    private final Fixture epubSmallIndexFixture;
    private final Fixture pdfSmallIndexFixture;
    ...
    ContentAccessGrantImpl(@Value("${tf.catalogue.base-url}") String baseUrl) {
        ...
        this.epubSmallIndexFixture = loadFixture(EPUB_SMALL_INDEX_FIXTURE);
        this.pdfSmallIndexFixture = loadFixture(PDF_SMALL_INDEX_FIXTURE);
    }
    ...
    public ContentGrant grant(ContentGrantRequest request) {
        ...
        // isEncrypted. Resolved separately from the content fixture (not content.url()) — see
        // resolveIndexFixture's own comment for why reusing content.url() was the bug.
        Fixture indexFixture = isAudio ? null : resolveIndexFixture(request.itemId());
        IndexUrl index = (request.wantSearchIndex() && indexFixture != null)
                ? new IndexUrl(baseUrl + "/" + indexFixture.path(), true, termCountFor(indexFixture))
                : null;
    ...
    /**
     * The real search-index fixture for an itemId, or null if it has none.
     *
     * ONLY the two small dev fixtures have one — see EPUB_SMALL_INDEX_FIXTURE's comment. Unlike
     * {@link #resolveFixture}, this has NO format-based fallback: an itemId with no index fixture
     * (a big measurement book, an unrecognised id) gets null and therefore no IndexUrl at all,
     * which is the honest "no index" the app already handles — never another book's content.
     */
    private Fixture resolveIndexFixture(String itemId) {
        if (EPUB_SMALL_FIXTURE.equals(ITEM_FIXTURE_MAP.get(itemId))) {
            return epubSmallIndexFixture;
        }
        if (PDF_SMALL_FIXTURE.equals(ITEM_FIXTURE_MAP.get(itemId))) {
            return pdfSmallIndexFixture;
        }
        return null;
    }

    private int termCountFor(Fixture indexFixture) {
        return indexFixture == epubSmallIndexFixture ? EPUB_TERM_COUNT : PDF_TERM_COUNT;
    }
