...
/**
 * One in-flight OIDC sign-in: an individual authenticating with no institution involved at all.
 *
 * <p>The OIDC counterpart of {@link com.tf.reader.auth.transaction.AuthTransaction}, and separate
 * from it for one reason: <b>the nonce</b>. It is bound into the ID token by the provider and
 * checked against what we sent, and it has no SAML analogue at all - widening the SAML record to
 * hold a field only OIDC uses would put a null in every SAML sign-in and invite somebody to start
 * reading it.
 *
