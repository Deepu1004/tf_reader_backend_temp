...
import com.tf.reader.auth.b2c.B2cTransactionStore;
...
/**
 * The complete local B2C flow, over real HTTP, with nothing stubbed.
 *
 * <pre>
 * POST /api/v1/auth/b2c/start (no body - no institution to name)
 *   → authorizationUrl
 *   → GET  {provider}/oauth2/authorize        (the sign-in page)
 *   → POST {provider}/oauth2/authorize        (the "Login &amp; Authorize" button)
 *   → 302  /api/v1/auth/b2c/callback?code=…&amp;state=…
 ...
 * </pre>
 *
 * <p>Unlike {@link OidcEndToEndAuthFlowTest}, the callback here is never read as a JSON body: it
 * is a browser redirect, exactly like the SAML ACS, so this test follows it by hand the same way
 * {@code SamlLoginFlowTest} does rather than the way the institutional OIDC test does.
 */
@SpringBootTest(
		webEnvironment = SpringBootTest.WebEnvironment.DEFINED_PORT,
		properties = { "tnf.auth.jwt.secret=" + ContainerisedInfrastructure.JWT_SECRET,
				"spring.profiles.active=" })
class B2cEndToEndAuthFlowTest extends MockOidcTestProfile {

	private static final String CODE_PREFIX =
			SamlAuthenticationSuccessHandler.DEEP_LINK_CALLBACK + "?code=";

	private static final String ERROR_DEEP_LINK =
			SamlAuthenticationSuccessHandler.DEEP_LINK_CALLBACK + "?error=OIDC_AUTHENTICATION_FAILED";
	...
	@Autowired
	private OidcTransactionStore oidcTransactions;

	@Autowired
	private B2cTransactionStore b2cTransactions;
	...
	void seedInstitutions() {
		...
		// Only the Coexistence tests' SAML leg needs a pre-provisioned user - B2C provisions its
	...
	@Test
	void aLocalB2cSignInBecomesAnApplicationJwtThatWorksOnProtectedApis() {
		Map<String, Object> start = startSignIn();

		assertThat(start.get("authTxnId").toString()).startsWith("b2cTxn_");
		...
		assertThat(callback).startsWith(B2C_REDIRECT_URI).contains("code=").contains("state=");
	...
	@Test
	void aB2cRefreshTokenDrivesRefreshAndLogoutLikeAnyOther() {
	...
	class Failures {
		...
		void aCallbackWithAStateWeNeverIssuedRedirectsWithAnError() {
			String deepLink = redirectLocation(B2C_REDIRECT_URI + "?code=made-up&state=never-issued");
		...
		void aCallbackWithAValidStateButNoCodeRedirectsWithAnError() {
			...
			assertThat(redirectLocation(B2C_REDIRECT_URI + "?state=" + state))
		...
		void aProviderErrorIsNotPassedThroughToTheClient() {
			String deepLink = redirectLocation(
					B2C_REDIRECT_URI + "?error=access_denied&error_description=THE-USER-CANCELLED&state=whatever");
	...
	// ───────────────────── all three sign-in flows in the same application ─────────────────────

	@Nested
	class Coexistence {

		@Test
		void aSamlSignInAndAB2cSignInResolveIndependently() {
			...
			Map<String, Object> tokens = exchangeCode(codeFrom(redirectLocation(callbackUrlFor())));
			String viaB2c = userIdFor((String) tokens.get("accessToken"));

			// Different accounts entirely: one is an institution membership, the other has none.
			assertThat(viaB2c).isNotEqualTo(viaSaml.user().userId());
			assertThat(viaSaml.user().institutionId()).isEqualTo(AuthTestInstitutions.UCL);
		}

		@Test
		void theThreeTransactionStoresDoNotShareIds() {
			// A state minted for one flow must be meaningless to another's callback.
			String samlTxn = samlTransactions.open(AuthTestInstitutions.UCL).id();
			String oidcTxn = oidcTransactions.open(AuthTestInstitutions.UCL).id();

			assertThat(redirectLocation(B2C_REDIRECT_URI + "?code=x&state=" + samlTxn))
					.isEqualTo(ERROR_DEEP_LINK);
			assertThat(redirectLocation(B2C_REDIRECT_URI + "?code=x&state=" + oidcTxn))
					.isEqualTo(ERROR_DEEP_LINK);
			assertThat(b2cTransactions.consume(samlTxn)).isEmpty();
			assertThat(b2cTransactions.consume(oidcTxn)).isEmpty();
		}
	}
	...
	private Map<String, Object> startSignIn() {
		return http.post().uri(uri("/api/v1/auth/b2c/start"))
				.retrieve().body(Map.class);
	}
	...
	private static Authentication samlAuthentication(String email) {
		Saml2ResponseAssertionAccessor assertion =
				new StubAssertion(email, Map.of(
